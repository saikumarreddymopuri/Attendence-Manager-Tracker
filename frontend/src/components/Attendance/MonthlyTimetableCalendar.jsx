    ...
    <div className="glass-panel p-6 rounded-[2rem] overflow-hidden">
      ...
      <div className="grid grid-cols-6 gap-2 mb-4">
        {dayLabels.map((day) => (
          <div
            key={day}
            className="text-center text-[10px] font-black text-slate-600 uppercase tracking-tighter"
          >
            {day}
          </div>
        ))}
            ...
            >
              <span
                className={`text-xs font-black mb-1 ${
                  isToday ? "text-neon-cyan" : "text-slate-500"
                }`}
              >
                {i + 1}
              </span>
              ...
              <div className="flex flex-col gap-1 overflow-hidden">
                {subjects.map((s, idx) => (
                  <div
                    key={idx}
                    className="text-[7px] leading-tight font-bold text-slate-300 bg-white/5 px-1 py-0.5 rounded truncate"
                  >
                    {s}
                  </div>
                ))}
